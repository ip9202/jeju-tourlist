...
export default function NewQuestionPage() {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.size > 5 * 1024 * 1024) {
      setErrors(prev => ({ ...prev, file: '파일 크기는 5MB를 초과할 수 없습니다' }));
    } else {
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              />
              {errors.title && (
                <div className="mt-2 flex items-center text-red-600" data-testid="title-error">
                  <AlertCircle className="w-4 h-4 mr-1" />
                  <span className="text-sm">{errors.title}</span>
                </div>
              )}
              {validation.title && (
                <div className="mt-2 flex items-center text-orange-600" data-testid="title-validation">
                  <AlertCircle className="w-4 h-4 mr-1" />
                  <span className="text-sm">{validation.title}</span>
                </div>
              )}
              {formData.title.length > 0 && !errors.title && !validation.title && (
            ...
            <div>
              ...
              />
              {errors.content && (
                <div className="mt-2 flex items-center text-red-600" data-testid="content-error">
                  <AlertCircle className="w-4 h-4 mr-1" />
                  <span className="text-sm">{errors.content}</span>
                </div>
              )}
              {validation.content && (
                <div className="mt-2 flex items-center text-orange-600" data-testid="content-validation">
                  <AlertCircle className="w-4 h-4 mr-1" />
            ...
            <div>
              ...
              />
              {errors.file && (
                <div className="mt-2 flex items-center text-red-600" data-testid="file-error">
                  <AlertCircle className="w-4 h-4 mr-1" />
