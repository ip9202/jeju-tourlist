...
test.describe('데스크톱 반응형 테스트', () => {
  
  test.beforeEach(async ({ page }) => {
    await page.goto('http://localhost:3000');
  });

  test.describe('데스크톱 (1024x768) - 표준 데스크톱', () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize({ width: 1024, height: 768 });
    });

    test('페이지가 데스크톱 화면에 맞게 조정됨', async ({ page }) => {
      // 가로 스크롤이 발생하지 않는지 확인
    ...
    test('헤더가 데스크톱에 최적화됨', async ({ page }) => {
    ...
    test('카드가 3열로 표시됨', async ({ page }) => {
      ...
      if (cardCount > 2) {
        ...
        const yDifference1 = Math.abs((firstCardBox?.y || 0) - (secondCardBox?.y || 0));
        const yDifference2 = Math.abs((secondCardBox?.y || 0) - (thirdCardBox?.y || 0));
        
        expect(yDifference1).toBeLessThan(10);
        expect(yDifference2).toBeLessThan(10);
      }
    });

    test('마우스 호버 효과가 작동함', async ({ page }) => {
      // 카드에 마우스 호버 효과가 있는지 확인
      const cards = page.locator('[data-testid="question-card"], .question-card, [class*="QuestionCard"]');
      const cardCount = await cards.count();
    ...
    test('텍스트가 데스크톱에 적절한 크기', async ({ page }) => {
      // 메인 제목이 데스크톱에 적절한 크기인지 확인
  ...
  test.describe('대형 데스크톱 (1920x1080) - FHD 모니터', () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize({ width: 1920, height: 1080 });
    });

    test('카드 레이아웃이 4열로 표시됨', async ({ page }) => {
      const cards = page.locator('[data-testid="question-card"], .question-card, [class*="QuestionCard"]');
      const cardCount = await cards.count();
      ...
      if (cardCount > 3) {
        ...
        const yDifference1 = Math.abs((firstCardBox?.y || 0) - (secondCardBox?.y || 0));
        const yDifference2 = Math.abs((secondCardBox?.y || 0) - (thirdCardBox?.y || 0));
        const yDifference3 = Math.abs((thirdCardBox?.y || 0) - (fourthCardBox?.y || 0));
        
        expect(yDifference1).toBeLessThan(10);
        expect(yDifference2).toBeLessThan(10);
        expect(yDifference3).toBeLessThan(10);
      }
    });

    test('텍스트가 대형 데스크톱에 적절한 크기', async ({ page }) => {
      // 본문 텍스트가 18px 이상인지 확인
    ...
    test('카드 너비가 적절함', async ({ page }) => {
      ...
      if (cardCount > 0) {
        ...
        // 카드 너비가 뷰포트의 20% 이상 30% 이하여야 함 (4열 레이아웃)
        const cardWidthRatio = (cardBox?.width || 0) / viewportWidth;
        expect(cardWidthRatio).toBeGreaterThanOrEqual(0.20);
        expect(cardWidthRatio).toBeLessThanOrEqual(0.30);
      }
    });

    test('네비게이션이 충분히 넓은 공간을 차지함', async ({ page }) => {
      const header = page.locator('header');
      const headerBox = await header.boundingBox();
      const viewportWidth = await page.evaluate(() => window.innerWidth);
      
      // 헤더가 뷰포트 너비의 80% 이상을 차지해야 함
      const headerWidthRatio = (headerBox?.width || 0) / viewportWidth;
      expect(headerWidthRatio).toBeGreaterThanOrEqual(0.80);
    });
  });

  test.describe('데스크톱 카드 컴포넌트 테스트', () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize({ width: 1024, height: 768 });
    });

    test('QuestionCard 데스크톱 최적화', async ({ page }) => {
    ...
    test('마우스 인터페이스 최적화', async ({ page }) => {
      // 모든 인터랙티브 요소가 마우스에 적합한 크기인지 확인
      ...
      for (let i = 0; i < Math.min(elementCount, 10); i++) {
        ...
        if (elementBox) {
          // 데스크톱에서는 최소 32px 이상이면 충분
          expect(elementBox.height).toBeGreaterThanOrEqual(32);
          expect(elementBox.width).toBeGreaterThanOrEqual(32);
        }
      }
    });
  ...
  test.describe('데스크톱 성능 테스트', () => {
    ...
    test('이미지 로딩 성능', async ({ page }) => {
      ...
      // 이미지 로딩이 3초 이내에 완료되어야 함
      expect(loadTime).toBeLessThan(3000);
    });

    test('애니메이션 성능', async ({ page }) => {
      // 호버 애니메이션이 부드럽게 작동하는지 확인
      const cards = page.locator('[data-testid="question-card"], .question-card, [class*="QuestionCard"]');
      const cardCount = await cards.count();
