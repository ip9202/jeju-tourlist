...
test.describe("로그인 테스트", () => {
  test("ip9202@gmail.com 계정으로 로그인 테스트", async ({ page }) => {
    ...
    const emailInput = page
      .locator(
        'input[type="email"], input[name="email"], input[placeholder*="이메일"], input[placeholder*="email"]'
      )
      .first();
    ...
    const passwordInput = page
      .locator(
        'input[type="password"], input[name="password"], input[placeholder*="비밀번호"], input[placeholder*="password"]'
      )
      .first();
    ...
    const loginButton = page
      .locator(
        'button[type="submit"], button:has-text("로그인"), button:has-text("Login"), input[type="submit"]'
      )
      .first();
    ...
    if (isLoggedIn) {
      ...
      const logoutButton = page.locator(
        'button:has-text("로그아웃"), a:has-text("로그아웃"), button:has-text("Logout")'
      );
      ...
      const errorMessage = page
        .locator('.error, .alert, [role="alert"], .text-red-500, .text-red-600')
        .first();
