...
describe('PopularSearchTerms', () => {
  ...
  it('should display keywords in text link format', () => {
    ...
    // Check that keywords are rendered as buttons (text links)
    mockKeywords.forEach(kw => {
      const button = screen.getByRole('button', { name: new RegExp(kw.keyword) });
      expect(button).toBeInTheDocument();
      expect(button).toHaveTextContent(kw.keyword);
    });
  });
  ...
  it('should have proper flex layout with separators', () => {
    ...
    // Check for separators (should be 4 separators for 5 keywords)
    const separators = container.querySelectorAll('span');
    // Some separators should have the · character
    const dotSeparators = Array.from(separators).filter(s => s.textContent === '·');
    expect(dotSeparators.length).toBe(4); // 5 keywords means 4 separators
  });
