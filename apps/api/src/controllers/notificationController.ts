...
export class NotificationController {
    ...
    try {
      ...
      if (result.count === 0) {
        return res.status(404).json(
          createResponse(false, '알림을 찾을 수 없습니다.', null)
        );
      }

      res.json(createResponse(true, '알림을 읽음으로 표시했습니다.', { count: result.count }));
    } catch (error) {
  ...
  markAllAsRead = async (req: Request, res: Response) => {
    try {
      ...
      res.json(createResponse(true, '모든 알림을 읽음으로 표시했습니다.', { count }));
    } catch (error) {
      console.error('모든 알림 읽음 처리 오류:', error);
      res.status(500).json(
        createResponse(false, '모든 알림 읽음 처리 중 오류가 발생했습니다.', null)
      );
    }
  };
  ...
  deleteNotification = async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json(
        createResponse(false, '알림 삭제 중 오류가 발생했습니다.', null)
      );
    }
  };
  ...
  updateSettings = async (req: Request, res: Response) => {
    try {
      ...
      const updatedSettings = await this.notificationService.updateUserNotificationSettings(
        userId,
        settings
      );

      res.json(createResponse(true, '알림 설정을 업데이트했습니다.', updatedSettings));
    } catch (error) {
      console.error('알림 설정 업데이트 오류:', error);
      res.status(500).json(
        createResponse(false, '알림 설정 업데이트 중 오류가 발생했습니다.', null)
      );
    }
  };
  ...
  getStats = async (req: Request, res: Response) => {
    try {
      ...
      const stats = await this.notificationService.getNotificationStats(userId, days);
  ...
  createNotification = async (req: Request, res: Response) => {
    try {
      ...
      res.status(201).json(createResponse(true, '알림을 생성했습니다.', result));
    } catch (error) {
      console.error('알림 생성 오류:', error);
      res.status(500).json(
        createResponse(false, '알림 생성 중 오류가 발생했습니다.', null)
      );
    }
  };
  ...
  createTemplate = async (req: Request, res: Response) => {
    try {
      ...
      if (!type) {
        return res.status(400).json(
          createResponse(false, '알림 타입이 필요합니다.', null)
        );
      }

      const template = await this.notificationService.createNotificationTemplate(type, data);

      res.json(createResponse(true, '알림 템플릿을 생성했습니다.', template));
    } catch (error) {
      console.error('알림 템플릿 생성 오류:', error);
      res.status(500).json(
        createResponse(false, '알림 템플릿 생성 중 오류가 발생했습니다.', null)
      );
    }
  };
  ...
  cleanupNotifications = async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json(
        createResponse(false, '알림 정리 중 오류가 발생했습니다.', null)
      );
    }
  };
}
