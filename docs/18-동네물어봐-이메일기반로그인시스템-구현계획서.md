...
- [ ] 2.1.1 `packages/validation/src/auth/register.schema.ts` 생성
  - [ ] `RegisterRequestSchema` 정의
  - [ ] 이메일 형식 검증 (RFC 5322)
  - [ ] 비밀번호 강도 검증 (최소 8자, 3종류 조합)
  - [ ] 닉네임 검증 (2-20자, 영문/한글/숫자)
- [ ] 2.1.2 품질 검증
  - [ ] `npm run lint` 통과 (packages/validation)
  - [ ] `npm run typecheck` 통과 (packages/validation)

##### 2.2 PasswordService 구현 (SRP - 비밀번호 처리 책임)

- [ ] 2.2.1 `apps/api/src/services/auth/PasswordService.ts` 생성
  - [ ] `hashPassword(password: string): Promise<string>` (bcrypt saltRounds: 10)
  - [ ] `comparePassword(password: string, hash: string): Promise<boolean>`
  - [ ] 비밀번호 강도 검증 로직
- [ ] 2.2.2 bcryptjs 패키지 설치
...
- [ ] 2.3.1 `apps/api/src/repositories/AuthRepository.ts` 생성
  - [ ] `findUserByEmail(email: string)` 메서드
  - [ ] `findUserByNickname(nickname: string)` 메서드
  - [ ] `createEmailUser(data: CreateEmailUserDTO)` 메서드
  - [ ] Prisma 트랜잭션 처리
- [ ] 2.3.2 품질 검증
  - [ ] `npm run lint` 통과 (apps/api)
  - [ ] `npm run typecheck` 통과 (apps/api)
...
- [ ] 2.4.1 `apps/api/src/services/auth/AuthService.ts` 생성
  - [ ] `registerWithEmail(data: RegisterDTO)` 메서드
  - [ ] 이메일 중복 체크 로직
  - [ ] 닉네임 중복 체크 로직
  - [ ] Repository + PasswordService 의존성 주입 (DIP)
- [ ] 2.4.2 품질 검증
  - [ ] `npm run lint` 통과 (apps/api)
  - [ ] `npm run typecheck` 통과 (apps/api)
...
- [ ] 2.5.1 `apps/api/src/routes/auth/register.ts` 생성
  - [ ] `POST /api/auth/register` 엔드포인트
  - [ ] Zod 스키마 검증 미들웨어
  - [ ] AuthService 호출
  - [ ] 에러 핸들링 (400, 409, 500)
- [ ] 2.5.2 라우터 등록
  - [ ] `apps/api/src/index.ts`에 라우트 추가
- [ ] 2.5.3 품질 검증
  - [ ] `npm run lint` 통과 (apps/api)
  - [ ] `npm run typecheck` 통과 (apps/api)
...
- [ ] 2.6.1 Postman/cURL 테스트
  - [ ] 정상 회원가입 시나리오 (201 Created)
  - [ ] 중복 이메일 에러 (409 Conflict)
  - [ ] 중복 닉네임 에러 (409 Conflict)
  - [ ] 잘못된 비밀번호 형식 (400 Bad Request)
- [ ] 2.6.2 전체 품질 검증
  - [ ] `npm run lint` 통과 (apps/api)
  - [ ] `npm run typecheck` 통과 (apps/api)
- [ ] 2.6.3 CLAUDE.md 업데이트 및 Git 커밋
  - [ ] CLAUDE.md에 Step 2 완료 기록
  - [ ] `git add .`
  - [ ] `git commit -m "feat: Step 2 - 회원가입 API 구현 (SOLID 원칙)"`
- [ ] 2.6.4 구현계획서 체크박스 업데이트
  - [ ] `docs/18-동네물어봐-이메일기반로그인시스템-구현계획서.md` Step 2 체크박스 모두 체크
  - [ ] `git add .`
  - [ ] `git commit -m "docs: Step 2 체크박스 업데이트"`
...
- [ ] 3.1.1 `apps/api/src/services/auth/AuthService.ts` 수정
  - [ ] `loginWithEmail(email, password)` 메서드
  - [ ] PasswordService.comparePassword() 호출
  - [ ] 로그인 실패 시 null 반환
- [ ] 3.1.2 품질 검증
  - [ ] `npm run lint` 통과 (apps/api)
  - [ ] `npm run typecheck` 통과 (apps/api)
...
- [ ] 3.2.1 `apps/api/src/routes/auth/login.ts` 생성
  - [ ] `POST /api/auth/login` 엔드포인트
  - [ ] AuthService.loginWithEmail() 호출
...
- [ ] 3.3.1 `apps/web/src/lib/auth.ts` 수정
  - [ ] `CredentialsProvider` 추가
  - [ ] `authorize()` 콜백 구현 (API 서버 `/api/auth/login` 호출)
  - [ ] 이메일/비밀번호 검증 로직
  - [ ] 기존 OAuth 설정 유지 (OCP)
- [ ] 3.3.2 품질 검증
...
- [ ] 3.4.1 기능 테스트
  ...
  - [ ] JWT 토큰 정상 발급 확인 (`/api/auth/session`)
- [ ] 3.4.2 전체 품질 검증
  - [ ] `npm run lint` 통과 (apps/api, apps/web)
  - [ ] `npm run typecheck` 통과 (apps/api, apps/web)
- [ ] 3.4.3 CLAUDE.md 업데이트 및 Git 커밋
  - [ ] CLAUDE.md에 Step 3 완료 기록
  - [ ] `git add .`
  - [ ] `git commit -m "feat: Step 3 - 로그인 API 및 NextAuth CredentialsProvider 구현"`
- [ ] 3.4.4 구현계획서 체크박스 업데이트
  - [ ] `docs/18-동네물어봐-이메일기반로그인시스템-구현계획서.md` Step 3 체크박스 모두 체크
  - [ ] `git add .`
  - [ ] `git commit -m "docs: Step 3 체크박스 업데이트"`
