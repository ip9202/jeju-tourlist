...
# - zh: "文档同步 + PR Ready转换"
argument-hint:
  "Mode target path - Mode: auto (default)|force|status|project, target
...
**Phase 2: Conditional Execution** (based on user choice) 4. **Document Synchronization**: Living Document updates and TAG integrity guaranteed (IF user selects "Proceed") 5. **Git operations**: Commit and PR state transitions via git-manager (IF user selects "Proceed")
